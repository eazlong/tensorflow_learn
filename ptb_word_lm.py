...
class PTBModel(object):
  ...
  def __init__(self, is_training, config, category):
...
class Predictor:
  _config = get_config()
  _initializer=None #=tf.random_uniform_initializer(-_config.init_scale, _config.init_scale)
  _category=None
  _checkpoint_dir = ''

  def __init__(self, category):
    self._category=category
    self._checkpoint_dir=category+"/"
  ...
  def train( self, train_data, vocab_list, retrain=False ):
    ...
    with tf.variable_scope(self._category, reuse=None, initializer=self._initializer), tf.Session() as session:
      ...
      for i in range(self._config.max_max_epoch):
        ...
        train_perplexity = self.__run_epoch(session, m, train_data, m.train_op,
                                     verbose=True)
        print("Epoch: %d Train Perplexity: %.3f" % (i + 1, train_perplexity))
        if i % 5 == 0:
          tf.train.Saver().save( session, self._checkpoint_dir+"model.ckpt" )
    

  def determine( self, sentenses ):
    perplexity = []
    with tf.variable_scope(self._category, reuse=True, initializer=self._initializer), tf.Session() as session:
      eval_config = get_config()
      eval_config.batch_size = 1
      eval_config.num_steps = 1
      mvalid = PTBModel(is_training=False, config=eval_config, category=self._category)
      ckpt = tf.train.get_checkpoint_state(self._checkpoint_dir)
      if ckpt and ckpt.model_checkpoint_path:
        tf.train.Saver().restore(session, ckpt.model_checkpoint_path)
      else:
        raise ValueError("check point file %s not found.", self._checkpoint_dir)

      for sentense in sentenses:
        p = self.__run_epoch(session, mvalid, sentense, tf.no_op())
        print( p )
        perplexity.append(p)
